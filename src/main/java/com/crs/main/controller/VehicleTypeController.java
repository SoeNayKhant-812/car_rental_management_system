...
public class VehicleTypeController {
    ...
    @GetMapping
    public ResponseEntity<List<VehicleTypeDTO>> getAllCars() {
        List<VehicleTypeDTO> cars = vehicleTypeService.getVehicleTypes();
        return ResponseEntity.ok(cars);
    }

    @GetMapping("/{id}")
    public ResponseEntity<VehicleTypeDTO> getCarById(@PathVariable Long id) {
        VehicleTypeDTO car = vehicleTypeService.getVehicleTypeById(id);
        return ResponseEntity.ok(car);
    }

    @PostMapping("/create")
    public ResponseEntity<VehicleTypeDTO> createCar(@RequestBody VehicleTypeDTO carDTO) {
        VehicleTypeDTO createdCar = vehicleTypeService.saveVehicleType(carDTO);
        return ResponseEntity.ok(createdCar);
    }

    @PutMapping("/{id}/update")
    public ResponseEntity<VehicleTypeDTO> updateCar(@PathVariable Long id,@RequestBody VehicleTypeDTO carDTO) {
